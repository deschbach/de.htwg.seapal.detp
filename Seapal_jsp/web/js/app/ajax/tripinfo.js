$(function() {
	...
	function loadEntry(waypnr) { 
		...
		jQuery.get("app_tripinfo_load.jsp", {'wnr': waypnr}, function(data) {
	...
	function addEntry(wnr, json) {
		...
		entry += "<a href='app_waypoint.jsp?wnr=" + wnr  + "' class='btn btn-small redirect' id='" + wnr + "'><span><i class='icon-chevron-right'></i></span></a>";
		entry += "</div></td>";
	    entry += "</tr>";
	...
	$('a.remove').live("click", function(event) {
		...
		jQuery.post("app_tripinfo_delete.jsp", { "wnr": waypnr }, function(data) { 
			...
			if (data['wnr'].match(/Error/)) {
		    	...
		    	$('#dialogMessage').text("Eintrag wurde erfolgreich geloescht.");
	    	}
	...
	$('#save').click(function(event) {
	    ...
	    jQuery.post("app_tripinfo_insert.jsp", json, function(data) { 
